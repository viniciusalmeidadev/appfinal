...
export default function App() {
    ...
    async function pegaComentario(){
        var user_id_desformatado = await AsyncStorage.getItem('@unitruckIdUserLogado')
        const user_id = JSON.parse(user_id_desformatado)
        const response = await api.get(`/consulta-comentario-edicao/${posto_id}&${user_id}`)
        .then(response =>{
            console.log(response.data)
            const {valor, descricao} = response.data[0]
    ...
    async function atualizarComentario(){
        ...
        const user_id = JSON.parse(user_id_desformatado)
        const posto_id = listaEstabelecimento.posto_id
        const response = await api.put('/atualizar-recomendacao-posto', {
    ...
    return(
        <View style={styles.containerPublicarComentario}>
            ...
            <TouchableOpacity style={styles.botaoPostar} onPress={()=>atualizarComentario()}>
                <Text style={{color:'#fff', fontSize:16, fontWeight:'bold'}}>Atualizar</Text>
            </TouchableOpacity>
